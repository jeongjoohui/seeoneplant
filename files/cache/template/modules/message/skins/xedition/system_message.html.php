...
<!--#Meta:modules/message/skins/xedition/css/message.css--><?php $__tmp=array('modules/message/skins/xedition/css/message.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
<div id="access">
	...
	<?php if(!$__Context->is_logged){ ?><div>
		<div class="login-body">
			...
			<?php Context::addJsFile("./files/ruleset/login.xml", FALSE, "", 0, "body", TRUE, "") ?><form id="message_login_form"  action="<?php echo getUrl('','act','procMemberLogin') ?>" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="@login" />
