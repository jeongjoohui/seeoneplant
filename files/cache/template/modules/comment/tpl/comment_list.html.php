...
<form id="fo_list" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<table id="commentListTable" class="x_table x_table-striped x_table-hover dsTg">
		<caption>
			<a href="<?php echo getUrl('','module','admin','act','dispCommentAdminList','Y') ?>"<?php if($__Context->search_keyword == ''){ ?> class="active"<?php } ?>><?php echo $lang->all;
...
if($__Context->search_target == 'is_secret' && $__Context->search_keyword == 'Y'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('search_target','is_published','search_keyword','N') ?>"<?php if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'N'){ ?> class="active"<?php } ?>><?php echo $lang->published_name_list['N'];
if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'N'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('search_target','is_published','search_keyword','Y') ?>"<?php if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'Y'){ ?> class="active"<?php } ?>><?php echo $lang->published_name_list['Y'];
if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'Y'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('', 'module', 'admin', 'act','dispCommentAdminDeclared') ?>"><?php echo $lang->cmd_declared_list ?></a>
			...
			<div class="x_btn-group x_pull-right">
				<a href="#listManager" class="x_btn modalAnchor" data-value="true" data-name="is_trash"><?php echo $lang->trash ?></a>
				<a href="#listManager" class="x_btn modalAnchor" data-value="false" data-name="is_trash"><?php echo $lang->delete ?></a>
				<?php if($__Context->search_target=='is_published' && $__Context->search_keyword=='Y'){ ?><a href="#listManager" class="x_btn modalAnchor" data-value="0" data-name="will_publish"><?php echo $lang->cmd_unpublish ?></a><?php } ?>
				<?php if($__Context->search_target=='is_published' && $__Context->search_keyword=='N'){ ?><a href="#listManager" class="x_btn modalAnchor" data-value="1" data-name="will_publish"><?php echo $lang->cmd_publish ?></a><?php } ?>
				<?php if($__Context->search_target!='is_published'){ ?><a href="#listManager" class="x_btn modalAnchor" data-value="0" data-name="will_publish"><?php echo $lang->cmd_unpublish ?></a><?php } ?>
				<?php if($__Context->search_target!='is_published'){ ?><a href="#listManager" class="x_btn modalAnchor" data-value="1" data-name="will_publish"><?php echo $lang->cmd_publish ?></a><?php } ?>
			</div>
		...
		<thead>
			<tr>
				...
				<th scope="col" class="nowr rx_detail_marks"><?php echo $lang->published ?></th>
				<th scope="col"><input type="checkbox" data-name="cart" title="Check All" /></th>
			</tr>
		</thead>
		<tbody>
			<?php if($__Context->comment_list)foreach($__Context->comment_list as $__Context->no=>$__Context->val){ ?><tr>
				<?php  $__Context->comment = $__Context->val->getContentText(200) ?>
				<td>
					<?php if(isset($__Context->module_list[$__Context->val->module_srl])){ ?>
					...
					<?php } ?>
					<a href="<?php echo getUrl('','document_srl',$__Context->val->document_srl,'comment_srl',$__Context->val->comment_srl) ?>#comment_<?php echo $__Context->val->comment_srl ?>" target="_blank"><?php if(strlen($__Context->comment)){;
echo $__Context->comment;
}else{ ?><em><?php echo $lang->no_text_comment ?></em><?php } ?></a>
				</td>
				<td class="nowr">
					<?php if($__Context->val->get('member_srl') <= 0){ ?><span><?php echo $__Context->val->getNickName() ?></span><?php } ?>
					<?php if($__Context->val->get('member_srl') < 0){ ?><a href="#popup_menu_area" class="member_<?php echo abs($__Context->val->get('member_srl')) ?>">(<?php echo $__Context->member_nick_name[abs($__Context->val->get('member_srl'))] ?>)</a><?php } ?>
					<?php if($__Context->val->get('member_srl') > 0){ ?><a href="#popup_menu_area" class="member_<?php echo $__Context->val->get('member_srl') ?>"><?php echo $__Context->val->getNickName() ?></a><?php } ?>
				</td>
				<td class="nowr rx_detail_marks"><?php echo number_format($__Context->val->get('voted_count')) ?>/<?php echo number_format($__Context->val->get('blamed_count')) ?></td>
				<td class="nowr">
					<span class="rx_detail_marks"><?php echo $__Context->val->getRegdate('Y-m-d H:i:s') ?></span>
					<span class="rx_simple_marks"><?php echo $__Context->val->getRegdate($__Context->val->getRegdateTime() > time() - 86400 ? 'H:i' : 'm-d') ?></span>
				</td>
				<td class="nowr rx_detail_marks"><a href="<?php echo getUrl('search_target','ipaddress','search_keyword',$__Context->val->ipaddress) ?>"><?php echo $__Context->val->ipaddress ?></a></td>
				<td class="nowr rx_detail_marks"><?php echo $__Context->secret_name_list[$__Context->val->isSecret() ? 'Y' : 'N'] ?></td>
				<td class="nowr rx_detail_marks"><?php echo $lang->published_name_list[$__Context->val->status] ?></td>
...
<form action="./" method="get" class="search center x_input-append" onsubmit="return checkSearch(this)"><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<select name="search_target" title="<?php echo $lang->search_target ?>" style="margin-right:4px">
		<?php if($lang->search_target_list)foreach($lang->search_target_list as $__Context->key => $__Context->val){ ?>
...
<?php Context::addJsFile("modules/comment/ruleset/deleteChecked.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_modal x" id="listManager"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="deleteChecked" />
	...
	<div class="x_modal-header">
		<h1><?php echo $lang->comment_manager ?>: <span class="_sub"></span></h1>
	</div>
	<div class="x_modal-body">
		<table id="commentManageListTable" class="x_table x_table-striped x_table-hover">
			<caption>
				<strong><?php echo $lang->selected_comment ?> <span id="selectedCommentCount"></span></strong>
			</caption>
			<thead>
				<tr>
					<th scope="col" class="title"><?php echo $lang->comment ?></th>
					<th scope="col" class="nowr"><?php echo $lang->writer ?></th>
	...
	<div class="x_modal-footer">
		<button type="submit" class="x_btn x_btn-inverse x_pull-right" name="is_trash|will_publish" value="true|false|0|1"><?php echo $lang->cmd_confirm ?></button>
	</div>
</form>
<script>
jQuery(function($){
	...
	$('a[data-value]').bind('before-open.mw', function(){
		if($docTable.find('tbody :checked').length == 0){
			...
			alert('<?php echo $lang->msg_not_selected_comment ?>');
			return false;
		} else {
			var $this = $(this);
