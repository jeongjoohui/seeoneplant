...
<table class="x_table x_table-striped x_table-hover">
	...
	<tbody>
		<?php if($__Context->layout_list)foreach($__Context->layout_list as $__Context->no=>$__Context->layout){ ?><tr>
...
<section id="insertLayout" class="x_modal">
	<?php Context::addJsFile("modules/layout/ruleset/insertLayout.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="./" method="post"  class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertLayout" />
		<input type="hidden" name="module" value="layout" />
		...
		<div class="x_modal-body">
			...
			<div class="x_control-group">
				...
				<div class="x_controls" style="padding-top:3px">
					<?php if($__Context->layout_info->author)foreach($__Context->layout_info->author as $__Context->author_info){ ?>
