...
if($__Context->comment->get('depth')){ ?> class="fbItem indent indent<?php echo ($__Context->comment->get('depth')) ?>"<?php } ?> id="comment_<?php echo $__Context->comment->comment_srl ?>">
			...
			<?php if($__Context->comment->status == RX_STATUS_DELETED){ ?>
				<div class="xe_content deleted"><?php echo $lang->msg_deleted_comment ?></div>
			<?php }elseif($__Context->comment->status == RX_STATUS_DELETED_BY_ADMIN){ ?>
				<div class="xe_content deleted deleted_by_admin"><?php echo $lang->msg_admin_deleted_comment ?></div>
			<?php }elseif(!$__Context->comment->isAccessible()){ ?>
			<form action="./" method="get" class="xe_content secretForm" onsubmit="return procFilter(this, input_password)"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
...
echo $__Context->file->download_url ?>"><?php echo $__Context->file->source_filename ?> <span class="fileSize">[File Size:<?php echo FileHandler::filesize($__Context->file->file_size) ?>/Download:<?php echo number_format($__Context->file->download_count) ?>]</span></a></li><?php } ?>
			...
			<?php if(!$__Context->comment->isDeleted()){ ?><p class="action">
				...
				<?php if($__Context->comment->getMyVote() === false || $__Context->comment->getMyVote() < 0){ ?><a href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteUp','<?php echo $__Context->comment->comment_srl ?>');return false;"<?php } ?> class="voted"><i class="xi-thumbs-up"></i><?php echo $lang->cmd_vote;
echo $__Context->comment->get('voted_count') ?></a><?php } ?>
				<?php if($__Context->comment->getMyVote() > 0){ ?><a href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteUpCancel','<?php echo $__Context->comment->comment_srl ?>');return false;"<?php } ?> class="voted"><i class="xi-thumbs-up"></i><?php echo $lang->cmd_vote;
echo $__Context->comment->get('voted_count') ?></a><?php } ?>
				<?php if($__Context->comment->getMyVote() === false || $__Context->comment->getMyVote() > 0){ ?><a href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteDown','<?php echo $__Context->comment->comment_srl ?>');return false;"<?php } ?> class="voted"><i class="xi-thumbs-up"></i><?php echo $lang->cmd_vote_down;
echo $__Context->comment->get('blamed_count') ?></a><?php } ?>
				<?php if($__Context->comment->getMyVote() < 0){ ?><a href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteDownCancel','<?php echo $__Context->comment->comment_srl ?>');return false;"<?php } ?> class="voted"><i class="xi-thumbs-up"></i><?php echo $lang->cmd_vote_down;
echo $__Context->comment->get('blamed_count') ?></a><?php } ?>
	...
	<?php if($__Context->grant->write_comment && $__Context->oDocument->isEnableComment()){ ?><form action="./" method="post" onsubmit="return procFilter(this, insert_comment)" class="write_comment" id="write_comment"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" />
