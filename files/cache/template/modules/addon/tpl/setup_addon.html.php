...
<table class="x_table">
	<tbody>
		...
		<tr>
			<th class="nowr"><?php echo $lang->author ?></th>
			<td>
				<?php if($__Context->addon_info->author)foreach($__Context->addon_info->author as $__Context->author){ ?>
		...
		<?php if($__Context->addon_info->homepage){ ?><tr>
			<th class="nowr"><?php echo $lang->homepage ?></th>
		...
		<?php if($__Context->addon_info->license){ ?><tr>
			<th class="nowr"><?php echo $lang->addon_license ?></th>
		...
		<?php if($__Context->addon_info->description){ ?><tr>
			<th class="nowr"><?php echo $lang->description ?></th>
			<td><?php echo $__Context->addon_info->description ?></td>
		</tr><?php } ?>
	</tbody>
</table>
<?php if(Context::isBlacklistedPlugin($__Context->addon_info->addon_name)){ ?><div class="message error">
	<p><em class="x_label x_label-important"><?php echo $lang->msg_warning ?></em> <?php echo $lang->msg_blacklisted_module ?><br /><?php echo $lang->get('admin.msg_blacklisted_reason.'.$__Context->addon_info->addon_name) ?></p>
</div><?php } ?>
...
<section class="section">
	...
	<?php if(count($__Context->addon_info->extra_vars)){ ?>
		<?php if($__Context->addon_info->extra_vars)foreach($__Context->addon_info->extra_vars as $__Context->id=>$__Context->var){ ?>
			<?php if($__Context->group != $__Context->var->group){ ?>
				<h2 style="margin-top:50px;"><?php echo $__Context->var->group ?></h2>
...
if($__Context->var->type == 'textarea'){ ?> for="lang_<?php echo $__Context->var->name ?>"<?php } ?>><?php echo $__Context->var->title ?></label>
				<div class="x_controls">
					<?php if($__Context->var->type == 'text'){ ?><input type="text" name="<?php echo $__Context->var->name ?>" id="<?php echo $__Context->var->name ?>" value="<?php echo escape($__Context->var->value) ?>"><?php } ?>
					<?php if($__Context->var->type == 'textarea'){ ?><textarea name="<?php echo $__Context->var->name ?>" id="<?php echo $__Context->var->name ?>" class="lang_code" rows="8" cols="42"><?php echo escape($__Context->var->value) ?></textarea><?php } ?>
					<?php if($__Context->var->type == 'select'){ ?><select name="<?php echo $__Context->var->name ?>" id="<?php echo $__Context->var->name ?>">
						<?php if($__Context->var->options)foreach($__Context->var->options as $__Context->option){ ?><option value="<?php echo $__Context->option->value ?>"<?php if($__Context->var->value == $__Context->option->value){ ?> selected="selected"<?php } ?>><?php echo $__Context->option->title ?></option><?php } ?>
					</select><?php } ?>
	...
	<?php if(!count($__Context->addon_info->extra_vars)){ ?><div class="message info">
		<p><?php echo $lang->msg_not_exist_option ?></p>
	</div><?php } ?>
</section>
<section class="section">
	<?php if($__Context->mid_list){ ?>
		<h1><?php echo $lang->target ?></h1>
		<div class="x_control-group"><?php echo $lang->about_addon_mid ?></div>
		<div class="x_control-group">
			<label class="x_control-label"><?php echo $lang->run_method ?></label>
			<div class="x_controls">
				<select name="xe_run_method">
					<option value="run_selected"<?php if($__Context->addon_info->xe_run_method === 'run_selected'){ ?> selected="selected"<?php } ?>><?php echo $lang->run_selected_module ?></option>
					<option value="no_run_selected"<?php if($__Context->addon_info->xe_run_method === 'no_run_selected'){ ?> selected="selected"<?php } ?>><?php echo $lang->no_run_selected_module ?></option>
				</select>
			</div>
		</div>
		<div class="x_control-group">
			...
			<div class="x_controls">
				...
				<?php if($__Context->mid_list)foreach($__Context->mid_list as $__Context->module_category_srl=>$__Context->modules){ ?>
				<?php if(count($__Context->mid_list) > 1){ ?>
				<fieldset>
					<legend><?php if($__Context->modules->title){;
echo $__Context->modules->title;
}else{;
echo $lang->none_category;
} ?></legend>
				<?php } ?>
					<?php if($__Context->modules->list)foreach($__Context->modules->list as $__Context->key=>$__Context->val){ ?><label>
