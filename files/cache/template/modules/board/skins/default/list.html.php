...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/default','_read.html');
} ?>
...
if($__Context->grant->manager){ ?> colspan="<?php echo count($__Context->list_config)+1 ?>"<?php } ?>>
		...
		<?php if($__Context->document_list || $__Context->notice_list){ ?><tbody>
			<!-- NOTICE -->
			<?php if($__Context->notice_list)foreach($__Context->notice_list as $__Context->no=>$__Context->document){ ?><tr class="notice">
				<?php if($__Context->list_config)foreach($__Context->list_config as $__Context->key=>$__Context->val){ ?>
...
echo $lang->notice;
			...
			<!-- /NOTICE -->
			<!-- LIST -->
			<?php if($__Context->document_list)foreach($__Context->document_list as $__Context->no=>$__Context->document){ ?><tr>
				<?php if($__Context->list_config)foreach($__Context->list_config as $__Context->key=>$__Context->val){ ?>
...
<div class="list_footer">
	<?php if($__Context->document_list || $__Context->notice_list){ ?><div class="pagination">
		<a href="<?php echo getUrl('page','','document_srl','','division',$__Context->division,'last_division',$__Context->last_division) ?>" class="direction prev"><span></span><span></span> <?php echo $lang->first_page ?></a> 
		...
		<?php } ?>
		<a href="<?php echo getUrl('page',$__Context->page_navigation->last_page,'document_srl','','division',$__Context->division,'last_division',$__Context->last_division) ?>" class="direction next"><?php echo $lang->last_page ?> <span></span><span></span></a>
	</div><?php } ?>
	<div class="btnArea">
		<a href="<?php echo getUrl('act','dispBoardWrite','document_srl','') ?>" class="btn"><?php echo $lang->cmd_write ?></a>
		<?php if($__Context->grant->manager){ ?><a href="<?php echo getUrl('','module','document','act','dispDocumentManageDocument') ?>" class="btn" onclick="popopen(this.href,'manageDocument'); return false;"><?php echo $lang->cmd_manage_document ?></a><?php } ?>
	</div>
	<button type="button" class="bsToggle" title="<?php echo $lang->cmd_search ?>"><?php echo $lang->cmd_search ?></button>
	<?php if($__Context->grant->view){ ?><form action="<?php echo getUrl() ?>" method="get" onsubmit="return procFilter(this, search)" id="board_search" class="board_search" ><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
		...
		<input type="text" name="search_keyword" value="<?php echo htmlspecialchars($__Context->search_keyword) ?>" title="<?php echo $lang->cmd_search ?>" class="iText" />
        ...
        <?php if($__Context->last_division){ ?><a href="<?php echo getUrl('page',1,'document_srl','','division',$__Context->last_division,'last_division','') ?>" class="btn"><?php echo $lang->cmd_search_next ?></a><?php } ?>
	</form><?php } ?>
	<a href="<?php echo getUrl('act','dispBoardTagList') ?>" class="tagSearch" title="<?php echo $lang->tag ?>"><?php echo $lang->tag ?></a>
</div>
