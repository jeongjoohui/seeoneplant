...
<p class="skipNav"><a href="#content"><?php echo $lang->skip_to_content ?></a></p>
	<header class="header">
		...
		<?php if($this->user->isAdmin()){ ?><p class="site">
			<a href="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars(getUrl(['module' => 'admin', 'act' => $__Context->act, 'module_srl' => $__Context->module_info->module_srl]), ENT_QUOTES, 'UTF-8', false) : (getUrl(['module' => 'admin', 'act' => $__Context->act, 'module_srl' => $__Context->module_info->module_srl]))) ?>"><?php echo lang('admin.view_in_manager_layout') ?></a>
		</p><?php } ?>
		<?php }else{ ?>
		<h1>
	...
	<!-- BODY -->
	<div class="body <?php if($_COOKIE['__xe_admin_gnb_status'] == 'close'){ ?>wide<?php } ?>"<?php if($__Context->module_manager){ ?> style="padding-left:0"<?php } ?>>
		<!-- GNB -->
		<?php if(!$__Context->module_manager){ ?><nav class="gnb <?php if($_COOKIE['__xe_admin_gnb_status'] == 'open'){ ?>open<?php } ?>" id="gnb">
			...
			<?php if($__Context->gnbUrlList)foreach($__Context->gnbUrlList as $__Context->key=>$__Context->value){ ?>
				...
				<li<?php if($_COOKIE['__xe_admin_gnb_tx_favorite'] == 'open'){ ?> class="open"<?php } ?>>
					<script>
					...
					<ul id="favorite"<?php if($_COOKIE['__xe_admin_gnb_tx_favorite'] == 'open'){ ?> style="display:block"<?php } ?>>
						<?php if($__Context->favorite_list)foreach($__Context->favorite_list as $__Context->favorite){ ?><li>
							...
							<form class="remove" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				...
				<?php } ?>
				<li class="<?php if(($__Context->parentSrl==$__Context->key || $__Context->value['href']=='index.php?module=admin') && !$__Context->mid && !$__Context->act){ ?>active open<?php }elseif($_COOKIE['__xe_admin_gnb_tx_' . md5($__Context->value['href'])] == 'open'){ ?>open<?php } ?>">
					<script>
					...
					<a href="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars(getFullUrl(''), ENT_QUOTES, 'UTF-8', false) : (getFullUrl('')));
echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value['href'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value['href'])) ?>" data-href="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars(md5($__Context->value['href']), ENT_QUOTES, 'UTF-8', false) : (md5($__Context->value['href']))) ?>" title="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value['text'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value['text'])) ?>"><span class="tx"><?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value['text'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value['text'])) ?></span></a>
					<?php if(count($__Context->value['list'])){ ?><ul<?php if($_COOKIE['__xe_admin_gnb_tx_' . md5($__Context->value['href'])] == 'open'){ ?> style="display:block"<?php } ?>>
						<?php if($__Context->value['list'])foreach($__Context->value['list'] as $__Context->key2=>$__Context->value2){;
if($__Context->value2['text']!=''){ ?><li<?php if($__Context->value2['text'] == $__Context->subMenuTitle){ ?> class="active_"<?php } ?> ><a href="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars(getFullUrl(''), ENT_QUOTES, 'UTF-8', false) : (getFullUrl('')));
echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value2['href'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value2['href'])) ?>" title="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value2['text'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value2['text'])) ?>"><?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->value2['text'], ENT_QUOTES, 'UTF-8', false) : ($__Context->value2['text'])) ?></a></li><?php }} ?>
					</ul><?php } ?>
				</li>
			<?php } ?>
			</ul>
