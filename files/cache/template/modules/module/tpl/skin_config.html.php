...
<form action="./" method="post" enctype="multipart/form-data" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" />
	...
	<section class="section">
		...
		<div class="x_control-group">
			...
			<div class="x_controls" style="padding-top:3px">
				<?php if($__Context->skin_info->author)foreach($__Context->skin_info->author as $__Context->author){ ?>
		...
		<?php if($__Context->skin_info->homepage){ ?><div class="x_control-group">
			<label class="x_control-label"><?php echo $lang->homepage ?></label>
		...
		<div class="x_control-group">
			<label class="x_control-label"><?php echo $lang->date ?></label>
		...
		<?php if($__Context->skin_info->license || $__Context->skin_info->license_link){ ?><div class="x_control-group">
			<label class="x_control-label"><?php echo $lang->skin_license ?></label>
			<div class="x_controls" style="padding-top:3px">
	...
	<section class="section">
		...
		<?php if($__Context->skin_info->colorset){ ?><div class="x_control-group">
			...
			<div class="x_controls">
				<?php if($__Context->skin_info->colorset)foreach($__Context->skin_info->colorset as $__Context->key=>$__Context->val){ ?>
					...
					<div<?php if($__Context->val->screenshot){ ?> class="x_thumbnail"<?php } ?> style="display:inline-block;width:<?php echo $__Context->_width ?>px;margin-right:10px;">
						...
						<?php if($__Context->val->screenshot){ ?><img src="/<?php echo $__Context->val->screenshot ?>" alt="<?php echo $__Context->val->title ?>" /><?php } ?>
					</div>
				<?php } ?>
			</div>
		</div><?php } ?>
		<?php if($__Context->skin_info->extra_vars)foreach($__Context->skin_info->extra_vars as $__Context->key=>$__Context->val){ ?>
...
if($__Context->val->options)foreach($__Context->val->options as $__Context->k=>$__Context->v){ ?><label for="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>" class="x_inline"><input type="radio" name="<?php echo $__Context->val->name ?>" value="<?php echo $__Context->v->value ?>" id="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>"<?php if($__Context->v->value==$__Context->val->value){ ?> checked="checked"<?php } ?> /> <?php echo $__Context->v->title ?></label><?php }} ?>
					...
					<?php if($__Context->val->type == 'image'){ ?>
						<?php if($__Context->val->value){ ?><div class="x_thumbnail" style="max-width:210px;margin:0 0 10px 0">
							...
							<label for="del_<?php echo $__Context->val->name ?>" style="padding:8px 0 0 0"><input type="checkbox" name="del_<?php echo $__Context->val->name ?>" value="Y" id="del_<?php echo $__Context->val->name ?>" class="checkbox" /> <?php echo $lang->cmd_delete ?></label>
						</div><?php } ?>
					...
					<?php if($__Context->val->type == 'colorpicker'){ ?><div>
						...
						<input type="text" class="rx-spectrum" name="<?php echo $__Context->val->name ?>" id="<?php echo $__Context->val->name ?>" value="<?php echo $__Context->val->value ?>" />
						<p id="categoy_color_help" hidden style="margin:8px 0 0 0"><?php echo $lang->about_category_color ?></p>
					</div><?php } ?>
					<?php if($__Context->val->description){ ?><p class="x_help-block"><?php echo nl2br(trim($__Context->val->description)) ?></p><?php } ?>
				</div>
			</div>
		<?php } ?>
	</section>
	<?php } ?>
	<div class="btnArea">
		<button class="x_btn x_btn-primary x_pull-right" type="submit"><?php echo $lang->cmd_registration ?></button>
	</div>
</form>
