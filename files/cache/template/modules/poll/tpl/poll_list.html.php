...
<?php Context::addJsFile("modules/poll/ruleset/deleteChecked.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" id="pollList"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="deleteChecked" />
	...
	<table class="x_table x_table-striped x_table-hover" id="pollListTable">
		...
		<tbody>
			<?php if($__Context->poll_list)foreach($__Context->poll_list as $__Context->no=>$__Context->val){ ?><tr>
...
<form action="./" method="get" class="search center x_input-append" style="clear:both;padding:10px 0 0 0;margin:0" ><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<select name="search_target" title="<?php echo $lang->search_target ?>" style="margin-right:4px">
		<?php if($lang->search_poll_target_list)foreach($lang->search_poll_target_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->key ?>"<?php if($__Context->search_target==$__Context->key){ ?> selected="selected"<?php } ?>><?php echo $__Context->val ?></option><?php } ?>
	</select>
