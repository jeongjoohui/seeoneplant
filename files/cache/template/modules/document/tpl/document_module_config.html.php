<?php if(!defined("__XE__"))exit;?><section class="section">
	...
	<form action="./" method="post" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<label class="x_inline" for="declared_message_admin"><input type="checkbox" name="declared_message[]" id="declared_message_admin" value="admin"<?php if($__Context->document_config->declared_message && in_array('admin', $__Context->document_config->declared_message)){ ?> checked="checked"<?php } ?> /> <?php echo $lang->cmd_declared_message_admin ?></label>
