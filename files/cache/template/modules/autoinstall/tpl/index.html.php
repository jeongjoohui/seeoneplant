...
<form action="" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<p>
		<?php if($__Context->show_ftp_note){ ?>
			<?php $__Context->ftp_link = sprintf('<a href="%s">%s</a>', getUrl('', 'module', 'admin', 'act', 'dispAdminConfigFtp'), $lang->ftp_setup) ?>
			<?php echo sprintf($lang->description_ftp_note, $__Context->ftp_link) ?>
		<?php } ?>
		<?php $__Context->btnUpdate = sprintf('<input type="submit" value="%s" class="x_btn x_btn-link" style="position:relative;top:-1px;text-decoration:underline !important" />', $lang->status_update) ?>
		<?php echo sprintf($lang->description_update, $__Context->btnUpdate) ?>
	</p>
</form>
-->
