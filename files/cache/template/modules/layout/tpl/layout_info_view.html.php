...
Context::addJsFile("modules/layout/ruleset/updateLayout.xml", FALSE, "", 0, "body", TRUE, "") ?><form id="config_form"  action="./" enctype="multipart/form-data" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="updateLayout" />
...
<section id="layout_config" class="x_form-horizontal">
	...
	<?php if($__Context->selected_layout->author){ ?><div class="x_control-group">
		...
		<div class="x_controls" style="padding-top:5px">
			<?php if($__Context->selected_layout->author)foreach($__Context->selected_layout->author as $__Context->author_info){ ?>
	...
	<div class="x_control-group">
		<label class="x_control-label" for="title"><?php echo $lang->title ?> <em>*</em></label>
		<div class="x_controls">
			<input type="text" id="title" name="title" value="<?php echo $__Context->selected_layout->layout_title ?>" />
			<span class="x_help-block"><?php echo $lang->about_title ?></span>
		</div>
	</div>
	<div class="x_control-group">
		<label class="x_control-label" for="header_script"><?php echo $lang->header_script ?></label>
		<div class="x_controls">
			<textarea name="header_script" id="header_script" rows="4" cols="42"><?php echo $__Context->selected_layout->header_script ?></textarea>
			<span class="x_help-block"><?php echo $lang->about_header_script ?></span>
		</div>
	</div>
	<?php if(!empty((array)$__Context->selected_layout->extra_var)){ ?><section class="section">
		<h1><?php echo $lang->extra_vars ?></h1>
		<?php $__Context->cnt = 1 ?>
		<?php if($__Context->selected_layout->extra_var)foreach($__Context->selected_layout->extra_var as $__Context->name=>$__Context->var){ ?>
...
echo $__Context->var->value;
					...
					<?php if($__Context->var->type == 'image'){ ?>
						...
						<div id="preview_<?php echo $__Context->name ?>" class="x_thumbnail" style="<?php if(!$__Context->var->value){ ?>display:none;<?php } ?>max-width:210px;margin-bottom:20px;">
							<img<?php if($__Context->var->value){ ?> src="/<?php echo $__Context->var->value ?>"<?php } ?> alt="" style="max-width:100%" />
							<div style="text-align:right">
								<button class="x_icon-remove" type="button" onclick="deleteImage('<?php echo $__Context->name ?>')" title="<?php echo $lang->cmd_delete ?>"><?php echo $lang->cmd_delete ?></button>
							</div>
						</div>
						<form action="./" enctype="multipart/form-data" method="post" target="hiddenIframe" class="imageUpload" style="margin:0"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
							...
							<p>
								<input type="file" name="img" id="file_select_<?php echo $__Context->name ?>" value="" title="<?php echo $lang->uploaded_file ?>" />
								<input class="x_btn" type="submit" value="<?php echo $lang->cmd_submit ?>" style="vertical-align:top" />
							</p>
						</form>
					<?php } ?>
					<?php if($__Context->var->type == 'select'){ ?><select name="<?php echo $__Context->name ?>" id="<?php echo $__Context->name ?>" style="width:auto">
						<?php if($__Context->var->options)foreach($__Context->var->options as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->key ?>"<?php if($__Context->key == $__Context->var->value){ ?> selected="selected"<?php } ?>><?php echo $__Context->val->val ?></option><?php } ?>
					</select><?php } ?>
					<?php if($__Context->var->type == 'checkbox'){ ?>
						<?php if($__Context->var->options)foreach($__Context->var->options as $__Context->key=>$__Context->val){ ?>
						...
						<div class="x_thumbnails">
							<?php if($__Context->var->options)foreach($__Context->var->options as $__Context->key=>$__Context->val){ ?>
								<div class="x_span2 <?php if($__Context->val->thumbnail){ ?>x_thumbnail<?php } ?>">
									<?php if($__Context->val->thumbnail){ ?><img src="/<?php echo $__Context->val->thumbnail ?>" alt="<?php echo $__Context->val->val ?>" /><?php } ?>
									<div class="x_caption">
					...
					<?php if($__Context->var->type == 'colorpicker'){ ?>
						...
						<input type="text" class="rx-spectrum" name="<?php echo $__Context->name ?>" id="<?php echo $__Context->name ?>" value="<?php echo $__Context->var->value ?>" />
						<p id="categoy_color_help" hidden style="margin:8px 0 0 0"><?php echo $lang->about_category_color ?></p>
					<?php } ?>
	...
	<section class="section">
		<?php if($__Context->selected_layout->menu){ ?><h1><?php echo $lang->menu ?></h1><?php } ?>
		<?php if($__Context->selected_layout->menu){ ?><div class="x_form-horizontal">
			<?php if($__Context->selected_layout->menu)foreach($__Context->selected_layout->menu as $__Context->menu_name=>$__Context->menu_info){ ?><div class="x_control-group">
				...
				<div class="x_controls">
					<select name="<?php echo $__Context->menu_name ?>" id="<?php echo $__Context->menu_name ?>">
						<option value="0"<?php if(!$__Context->menu_info->menu_srl){ ?> selected="selected"<?php } ?>><?php echo $lang->cmd_select ?></option>
						<option<?php if($__Context->menu_info->menu_srl == -1){ ?> selected="selected"<?php } ?> value="-1"><?php echo $lang->sitemap_with_homemenu ?></option>
						<?php if($__Context->menu_list)foreach($__Context->menu_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->val->menu_srl ?>"<?php if($__Context->val->menu_srl == $__Context->menu_info->menu_srl){ ?> selected="selected"<?php } ?>><?php echo $__Context->val->title ?></option><?php } ?>
					</select>
				</div>
			</div><?php } ?>
			<div class="x_control-group">
				<label class="x_control-label"><?php echo $lang->not_apply_menu ?></label>
				<div class="x_controls">
					<label class="x_inline"><input type="checkbox" name="apply_layout" id="apply_layout" value="Y" /> <?php echo $lang->about_not_apply_menu ?></label>
				</div>
			</div>
		</div><?php } ?>
	</section>
	<div class="x_clearfix btnArea">
		<span class="etc">
			<?php if($__Context->layout){ ?><a class="x_btn" href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispLayoutAdminInstanceList', 'layout', $__Context->selected_layout->layout, 'type', $__Context->type) ?>"><?php echo $lang->cmd_list ?></a><?php } ?>
			<?php if(!$__Context->layout){ ?><a class="x_btn" href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispLayoutAdminAllInstanceList', 'type', $__Context->type) ?>"><?php echo $lang->cmd_list ?></a><?php } ?>
		</span>
		<span class="x_btn-group x_pull-right">
			<input class="x_btn x_btn-primary" type="submit" value="<?php echo $lang->cmd_save ?>" onclick="doSubmitConfig()"/>
		</span>
	</div>
</section>
