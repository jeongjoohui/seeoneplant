...
$query = new Query();
$query->setQueryId("getRemoteRanking");
...
new SelectExpression('`remote`')
,new SelectExpression('`count`', '`cnt`')
));
$query->setTables(array(
new Table('`rx_referer_remote_statistics`', '`referer_remote_statistics`')
));
$query->setConditions(array(
new ConditionGroup(array(
new ConditionWithArgument('`remote`',$search_keyword1_argument,"like", 'and')),'and')
));
