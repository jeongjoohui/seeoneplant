...
$__Context->_popular_order_type = 'desc';
		...
		<?php } ?>
		<a href="<?php echo getUrl('order_target','newest','order_type',$__Context->_newest_order_type,page,'') ?>"<?php if($__Context->order_target == 'newest'){ ?> class="active"<?php } ?>><?php echo $lang->order_newest ?> <?php if($__Context->order_target == 'newest'){ ?><i<?php if($__Context->_newest_order_type == 'asc'){ ?> class="x_icon-circle-arrow-down"<?php };
if($__Context->_newest_order_type == 'desc'){ ?> class="x_icon-circle-arrow-up"<?php } ?>><?php echo $__Context->_newest_order_type ?></i><?php } ?></a>
		<i>|</i>
		<a href="<?php echo getUrl('order_target','download','order_type',$__Context->_download_order_type,page,'') ?>"<?php if($__Context->order_target == 'download'){ ?> class="active"<?php } ?>><?php echo $lang->order_download ?> <?php if($__Context->order_target == 'download'){ ?><i<?php if($__Context->_download_order_type == 'asc'){ ?> class="x_icon-circle-arrow-down"<?php };
if($__Context->_download_order_type == 'desc'){ ?> class="x_icon-circle-arrow-up"<?php } ?>><?php echo $__Context->_download_order_type ?></i><?php } ?></a>
		<i>|</i>
		<a href="<?php echo getUrl('order_target','popular','order_type',$__Context->_popular_order_type,page,'') ?>"<?php if($__Context->order_target == 'popular'){ ?> class="active"<?php } ?>><?php echo $lang->order_popular ?> <?php if($__Context->order_target == 'popular'){ ?><i<?php if($__Context->_popular_order_type == 'asc'){ ?> class="x_icon-circle-arrow-down"<?php };
...
echo $__Context->current_category;
		...
		<div class="x_btn-group x_pull-right">
			<button class="x_btn x_active __btn_simple"><?php echo $lang->simple_view ?></button>
			<button class="x_btn __btn_detail"><?php echo $lang->detail_view ?></button>
		</div>
	</div>
	<div class="list">
		<?php if($__Context->item_list)foreach($__Context->item_list as $__Context->key=>$__Context->item){ ?>
...
echo sprintf($lang->rate, $__Context->item->package_start) ?></span></span>
					...
					<p class="x_btn-group" style="text-align:right">
						<?php if($__Context->item->current_version && $__Context->item->need_update != 'Y'){ ?><a class="x_btn x_disabled" href="#"><?php echo $lang->installed ?></a><?php } ?>
						<?php if(!$__Context->item->current_version){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminInstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-download-alt"></i> <?php echo $lang->install ?></a><?php } ?>
						<?php if($__Context->item->current_version && $__Context->item->avail_remove){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminUninstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-remove"></i> <?php echo $lang->cmd_delete ?></a><?php } ?>
						<?php if($__Context->item->current_version && $__Context->item->need_update == 'Y'){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminInstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-refresh"></i> <?php echo $lang->update ?></a><?php } ?>
					</p>
				</div>
			</div>
		<?php } ?>
...
echo $__Context->current_category;
	...
	<thead>
		<tr>
			<th scope="col" class="nowr"><?php echo $lang->thumbnail ?></th>
			<th scope="col" class="nowr"><?php echo $lang->name ?></th>
			<th scope="col" class="nowr"><?php echo $lang->distribute_version ?></th>
			<th scope="col" class="nowr"><?php echo $lang->current_version ?></th>
			<th scope="col" class="nowr"><?php echo $lang->run ?></th>
		</tr>
	</thead>
	<tbody>
		<?php if($__Context->item_list)foreach($__Context->item_list as $__Context->key=>$__Context->item){ ?><tr>
			<?php  $__Context->target_url = $__Context->original_site."?mid=download&package_srl=".$__Context->item->package_srl;  ?>
			<td>
...
echo sprintf($lang->rate, $__Context->item->package_start) ?></span></span>
					...
					<?php if((int)$__Context->item->package_voter <= 0){ ?>
						...
						<span class="x_badge">0/0</span>
					<?php } ?>
					<?php echo $lang->package_update ?>: <time title="<?php echo zdate($__Context->item->item_regdate, "Y-m-d H:i") ?>"><?php echo zdate($__Context->item->item_regdate, "Y-m-d") ?></time> 
					<i>|</i>
					<?php echo $lang->package_downloaded_count ?>: <?php echo number_format($__Context->item->package_downloaded) ?>
					<i>|</i>
				</p>
				<?php if($__Context->item->deps){ ?><p><?php echo $lang->dependant_list ?>:</p><?php } ?>
				<?php if($__Context->item->deps){ ?><ul class="">
					<?php if($__Context->item->deps)foreach($__Context->item->deps as $__Context->package_srl){ ?><li><?php echo $__Context->installed[$__Context->package_srl]->title ?></li><?php } ?>
				</ul><?php } ?>
			</td>
			...
			<td>
				<span class="x_btn-group">
					<?php if($__Context->item->current_version && $__Context->item->need_update != 'Y'){ ?><a class="x_btn x_disabled" href="#"><?php echo $lang->installed ?></a><?php } ?>
					<?php if(!$__Context->item->current_version){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminInstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-download-alt"></i> <?php echo $lang->install ?></a><?php } ?>
					<?php if(!$__Context->show_ftp_note && $__Context->item->current_version && $__Context->item->avail_remove){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminUninstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-remove"></i> <?php echo $lang->cmd_delete ?></a><?php } ?>
					<?php if($__Context->item->current_version && $__Context->item->need_update == 'Y'){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminInstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-refresh"></i> <?php echo $lang->update ?></a><?php } ?>
				</span>
			</td>
		</tr><?php } ?>
...
<div class="x_clearfix" style="margin-top:-10px">
	<?php if($__Context->page_navigation){ ?><form action="./" class="x_pagination x_pull-left" ><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		...
		<input type="hidden" name="order_target" value="<?php echo $__Context->order_target ?>" />
		<input type="hidden" name="order_type" value="<?php echo $__Context->order_type ?>" />
	...
	<?php if($__Context->act!=dispAutoinstallAdminInstalledPackages){ ?><form action="./" class="search x_input-append x_pull-right" ><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		...
		<input type="hidden" name="order_target" value="<?php echo $__Context->order_target ?>" />
		<input type="hidden" name="order_type" value="<?php echo $__Context->order_type ?>" />
		<input type="search" name="search_keyword" required title="Search" value="<?php echo $__Context->search_keyword ?>" />
		<button class="x_btn x_btn-inverse" type="submit"><?php echo $lang->cmd_search ?></button>
		<a href="<?php echo getUrl('page', '', 'search_keyword', '') ?>" class="x_btn"><?php echo $lang->cmd_cancel ?></a>
	</form><?php } ?>
</div>
<script>
jQuery(function($){
