...
<div class="x_page-header">
	<h1><?php echo $lang->menu_gnb_sub['filebox'] ?></h1>
</div>
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/module/tpl/adminFileBox/1'){ ?><div class="message <?php echo $__Context->XE_VALIDATOR_MESSAGE_TYPE ?>">
	<p><?php echo $__Context->XE_VALIDATOR_MESSAGE ?></p>
</div><?php } ?>
<section class="section">
	<h1><?php echo $lang->upload_file ?></h1>
	<form action="./" class="x_form-horizontal" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" />
		<input type="hidden" name="module" value="module" />
		<input type="hidden" name="act" value="procModuleFileBoxAdd" />
		<input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<input type="hidden" name="filter" value="<?php echo $__Context->filter ?>" />
		<input type="hidden" name="input" value="<?php echo $__Context->input ?>" />
		...
		<div class="x_control-group __attribute" data-count="1">
			<label for="attribute_name1" class="x_control-label __attribute_name_label"><?php echo $lang->attribute_name ?></label>
			<div class="x_controls">
				<input type="text" name="attribute_name[]" id="attribute_name1" class="__attribute_name" style="margin-right:20px" />
				<label for="attribute_value1" class="x_inline __attribute_value_label"><?php echo $lang->attribute_value ?></label>
				...
				<span class="x_btn-group">
					<button type="button" class="x_btn __addBtn" onclick="addRow('fileUp');"><?php echo $lang->cmd_add ?></button>
					<button type="button" class="x_btn __deleteBtn" onclick="clearRow(this);"><?php echo $lang->cmd_delete ?></button>
				</span>
			</div>
		</div>
		<div class="x_control-group">
			<label for="file" class="x_control-label"><?php echo $lang->file ?></label>
			<div class="x_controls">
...
<section class="section">
	<h1><?php echo $lang->filebox_list ?></h1>
	<table class="x_table x_table-striped x_table-hover">
		<thead>
			<tr>
				<th scope="col"><?php echo $lang->file ?></th>
				<th scope="col"><?php echo $lang->attribute_name ?>:<?php echo $lang->attribute_value ?></th>
				<th scope="col"><?php echo $lang->cmd_delete ?></th>
			</tr>
		</thead>
		<tbody>
			<?php if($__Context->filebox_list)foreach($__Context->filebox_list as $__Context->key=>$__Context->val){ ?><tr>
				<td>
					<?php if($__Context->val->fileextension=='swf'){ ?>
						<object height="100" width="100" flashvars="" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" xmlns="http://www.w3.org/1999/xhtml">
							<param value="<?php echo getUrl('');
echo $__Context->val->filename ?>" name="movie" xmlns="http://www.w3.org/1999/xhtml" />
							...
							<embed height="100" width="100" wmode="transparent" flashvars="" autostart="false" src="<?php echo getUrl('');
echo $__Context->val->filename ?>" xmlns="http://www.w3.org/1999/xhtml" />
						</object>
					<?php }elseif(in_array($__Context->val->fileextension,array('gif','png','jpg','jpeg'))){ ?>
						<img src="<?php echo getUrl('');
echo $__Context->val->filename ?>" style="max-height:60px" />
					<?php }else{ ?>
						<a href="<?php echo getUrl('');
echo $__Context->val->filename ?>"><?php echo basename($__Context->val->filename) ?></a>
					<?php } ?>
				</td>
				<td>
					<?php if($__Context->val->attributes)foreach($__Context->val->attributes as $__Context->name=>$__Context->value){ ?><p>
						<?php echo htmlspecialchars($__Context->name, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>: <?php echo $__Context->value ?>
					</p><?php } ?>
				</td>
				<td>
					<button type="button" class="x_btn x_btn-link" style="white-space:nowrap" onclick="XE.filebox.deleteFile('<?php echo $__Context->val->module_filebox_srl ?>');"><?php echo $lang->cmd_delete ?></button>
				</td>
			</tr><?php } ?>
		</tbody>
	</table>
