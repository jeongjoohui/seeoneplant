...
<?php if($__Context->act == 'dispAutoinstallAdminIndex'){ ?><nav class="x_thumbnail x_clearfix category">
	<?php if($__Context->categories)foreach($__Context->categories as $__Context->category){;
if($__Context->category->depth == 0){ ?><div>
		<h2><?php echo $__Context->category->title ?></h2>
		<ul>
			<?php if($__Context->category->children)foreach($__Context->category->children as $__Context->children){;
