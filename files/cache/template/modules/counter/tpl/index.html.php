...
<!--#Meta:modules/counter/tpl/counter.css--><?php $__tmp=array('modules/counter/tpl/counter.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
...
<div class="x_page-header">
	<h1><?php echo $lang->counter ?> <?php echo $lang->cmd_management ?></h1>
</div>
<form action="./index.php" method="get" name="calendar" id="fo_calendar" style="margin-bottom:30px"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<table class="x_table x_table-striped x_table-hover">
		<thead>
			<tr>
				<th scope="row"><?php echo $lang->total_counter ?></th>
				<th scope="row"> 
...
<ul class="x_nav x_nav-tabs">
	<?php if($lang->cmd_select_counter_type)foreach($lang->cmd_select_counter_type as $__Context->key=>$__Context->val){ ?><li<?php if($__Context->type==$__Context->key || (!$__Context->type && $__Context->key == 'hour')){ ?> class="x_active"<?php } ?>>
		<a href="<?php echo getUrl('type',$__Context->key) ?>"><?php echo $__Context->val ?></a> 
	</li><?php } ?>
</ul>
<!-- 일자를 옮기는 form -->
<form action="./" method="get" id="fo_counter"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<input type="hidden" name="module" value="<?php echo $__Context->module ?>" />
	<input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
...
<!-- unique visitor 그래프 -->
<table class="x_table x_table-striped x_table-hover">
	<?php if($__Context->detail_status->list)foreach($__Context->detail_status->list as $__Context->key => $__Context->val){ ?>
	...
	<tr>
		<?php if(Context::getLangType()=='en'){ ?><th scope="row"> 
			...
			<?php }elseif($__Context->type == 'day'){ ?>
				<?php echo $__Context->key ?> <?php echo $lang->unit_day ?>
			<?php }else{ ?>
				<?php echo $__Context->key ?>
			<?php } ?>
		</th><?php } ?>
		<?php if(Context::getLangType()!='en'){ ?><th scope="row"> 
			<?php if($__Context->type == 'year'){ ?>
				<?php echo $__Context->key ?> <?php echo $lang->unit_year ?>
			<?php }elseif($__Context->type == 'month'){ ?>
				<?php echo $__Context->key ?> <?php echo $lang->unit_month ?>
			<?php }elseif($__Context->type == 'day'){ ?>
				<?php echo $__Context->key ?> <?php echo $lang->unit_day ?>
			<?php }else{ ?>
				<?php echo $__Context->key ?> <?php echo $lang->unit_hour ?>
			<?php } ?>
		</th><?php } ?>
