...
$query = new Query();
$query->setQueryId("getUAgentRanking");
...
new SelectExpression('`uagent`')
,new SelectExpression('`count`', '`cnt`')
));
$query->setTables(array(
new Table('`rx_referer_uagent_statistics`', '`referer_uagent_statistics`')
));
$query->setConditions(array(
new ConditionGroup(array(
new ConditionWithArgument('`uagent`',$search_keyword1_argument,"like", 'and')),'and')
));
