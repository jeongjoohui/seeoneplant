...
<?php Context::addJsFile("modules/board/ruleset/insertBoard.xml", FALSE, "", 0, "body", TRUE, "") ?><form  class="x_form-horizontal" action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertBoard" />
...
if(!$__Context->list_config[$__Context->key]){ ?><option value="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->key, ENT_QUOTES, 'UTF-8', false) : ($__Context->key)) ?>"><?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->val->name, ENT_QUOTES, 'UTF-8', false) : ($__Context->val->name)) ?></option><?php }} ?>
					</select><br />
				...
				<div style="display:inline-block">
					<select class="multiorder_selected" size="8" multiple="multiple" style="width:220px;vertical-align:top;margin-bottom:8px">
						<?php if($__Context->list_config)foreach($__Context->list_config as $__Context->key=>$__Context->val){ ?><option value="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->key, ENT_QUOTES, 'UTF-8', false) : ($__Context->key)) ?>"><?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->val->name, ENT_QUOTES, 'UTF-8', false) : ($__Context->val->name)) ?></option><?php } ?>
					</select><br />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<select name="order_target" id="order_target" title="<?php echo $lang->order_target ?>">
					...
					<?php if($__Context->extra_order_target)foreach($__Context->extra_order_target as $__Context->key=>$__Context->val){ ?><option value="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->key, ENT_QUOTES, 'UTF-8', false) : ($__Context->key)) ?>"<?php if($__Context->module_info->order_target== $__Context->key){ ?> selected="selected"<?php } ?>><?php echo ($this->config->autoescape === 'on' ? htmlspecialchars($__Context->val, ENT_QUOTES, 'UTF-8', false) : ($__Context->val)) ?></option><?php } ?>
					<?php } ?>
				</select>
	...
	<div class="x_clearfix btnArea">
		<div class="x_pull-left">
			<a href="<?php echo ($this->config->autoescape === 'on' ? htmlspecialchars(getUrl('act', 'dispBoardAdminContent'), ENT_QUOTES, 'UTF-8', false) : (getUrl('act', 'dispBoardAdminContent'))) ?>" type="button" class="x_btn"><?php echo $lang->cmd_cancel ?></a>
		</div>
