...
<?php Context::addJsFile("modules/member/ruleset/insertLoginConfig.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="./" class="x_form-horizontal"  method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertLoginConfig" />
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<label class="x_inline" for="identifiers_user_id">
				<input type="checkbox" name="identifiers[]" id="identifiers_user_id" value="user_id"<?php if($__Context->config->identifier === 'user_id' || in_array('user_id', $__Context->config->identifiers)){ ?> checked="checked"<?php } ?> />
				<?php echo $lang->user_id ?>
			</label>
			<label class="x_inline" for="identifiers_email_address">
				<input type="checkbox" name="identifiers[]" id="identifiers_email_address" value="email_address"<?php if($__Context->config->identifier === 'email_address' || in_array('email_address', $__Context->config->identifiers)){ ?> checked="checked"<?php } ?> />
				<?php echo $lang->email_address ?>
			</label>
			<label class="x_inline" for="identifiers_phone_number">
				<input type="checkbox" name="identifiers[]" id="identifiers_phone_number" value="phone_number"<?php if($__Context->config->identifier === 'phone_number' || in_array('phone_number', $__Context->config->identifiers)){ ?> checked="checked"<?php } ?> />
