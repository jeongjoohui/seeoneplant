...
<!--#Meta:modules/editor/skins/ckeditor/css/default.less?$__Context->css_var--><?php $__tmp=array('modules/editor/skins/ckeditor/css/default.less','','','',$__Context->css_var);Context::loadFile($__tmp);unset($__tmp); ?>
...
<input type="hidden" name="_saved_doc_title" value="<?php echo escape($__Context->saved_doc->title) ?>" />
<input type="hidden" name="_saved_doc_content" value="<?php echo escape($__Context->saved_doc->content) ?>" />
...
<script>
	jQuery(function($){
		...
		<?php if($lang->edit->fontlist)foreach($lang->edit->fontlist as $__Context->fontname){ ?>
			font_list.push(<?php echo json_encode($__Context->fontname) ?>);
		<?php } ?>
		...
		var default_font_size = <?php echo json_encode($__Context->content_font_size ?: '13') ?>;
