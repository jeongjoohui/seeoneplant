...
<form id="fo_list" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<table id="fileListTable" class="x_table x_table-striped x_table-hover">
		<caption>
			<a href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispFileAdminList') ?>"<?php if(!$__Context->isvalid){ ?> class="active"<?php } ?>><?php echo $lang->all;
if(!$__Context->isvalid){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('isvalid', 'Y') ?>"<?php if($__Context->isvalid == 'Y'){ ?> class="active"<?php } ?>><?php echo $lang->is_valid;
if($__Context->isvalid == 'Y'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('isvalid', 'N') ?>"<?php if($__Context->isvalid == 'N'){ ?> class="active"<?php } ?>><?php echo $lang->is_stand_by;
if($__Context->isvalid == 'N'){ ?>(<?php echo number_format($__Context->total_count) ?>)<?php } ?></a>
			
			<span class="x_pull-right"><a href="#listManager" class="x_btn modalAnchor" onclick="getFileList();"><?php echo $lang->delete ?></a></span>
		</caption>
		<thead>
			<tr>
				<th scope="col"><?php echo $lang->file ?></th>
				<th scope="col" class="nowr"><?php echo $lang->file_size ?></th>
				<th scope="col" class="nowr"><?php echo $lang->cmd_download ?></th>
		...
		<tbody>
			<?php if($__Context->file_list)foreach($__Context->file_list as $__Context->no => $__Context->val){ ?>
				<!-- one document start -->
				...
				<th colspan="8" scope="col">
					<?php if(!$__Context->val->upload_target_type){ ?>
						<?php if($__Context->val->isvalid=='Y'){ ?>
							<?php echo $lang->is_valid ?>
						<?php }else{ ?>
							<?php echo $lang->is_stand_by ?>
						<?php } ?>
					<?php }else{ ?>
						<?php if($__Context->val->upload_target_type == 'com'){ ?>[<?php echo $lang->comment ?>] <?php } ?>
						<?php if($__Context->val->upload_target_type == 'mod'){ ?>[<?php echo $lang->module ?>] <?php } ?>
						<?php if($__Context->val->upload_target_type == 'msg'){ ?>[<?php echo $lang->member_message ?>] <?php } ?>
						<?php if($__Context->val->upload_target_type == 'doc' && $__Context->document_list[$__Context->document_srl] && $__Context->document_list[$__Context->document_srl]->get('module_srl') == $__Context->document_list[$__Context->document_srl]->get('member_srl')){ ?>[<?php echo $lang->cmd_temp_save ?>] <?php } ?>
						<?php if($__Context->val->upload_target_type == 'doc' && $__Context->document_list[$__Context->document_srl] && $__Context->document_list[$__Context->document_srl]->get('module_srl') == 0){ ?>[<?php echo $lang->cmd_trash ?>] <?php } ?>
						<?php if(in_array($__Context->val->upload_target_type, ['doc', 'com', 'mod'])){ ?>
...
echo $__Context->document_list[$__Context->document_srl]->getTitle();
				...
				<!-- one document end -->
			<tr>
				<td>	<?php if(preg_match("/\.(jpg|png|bmp|jpeg|gif)$/i",strtolower($__Context->val->source_filename))){ ?>
					<img src="<?php echo $__Context->val->uploaded_filename ?>" style="width:50px; height:50px">
					<?php } ?>
					<a href="<?php echo htmlspecialchars_decode($__Context->val->download_url) ?>"><?php echo htmlspecialchars($__Context->val->source_filename, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?></a></td>
...
echo $lang->is_stand_by;
			...
			<?php if(!$__Context->file_list){ ?><tr>
				<td><?php echo $lang->no_files ?></td>
			</tr><?php } ?>
		</tbody>
	</table>
	<span class="x_pull-right">
		<a href="#listManager" class="x_btn modalAnchor" onclick="getFileList();"><?php echo $lang->delete ?></a>
	</span>
</form>
...
<form action="./" method="get" onsubmit="return checkSearch(this)" class="search center x_input-append"><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<select name="search_target" title="<?php echo $lang->search_target ?>" style="margin-right:4px">
		<?php if($lang->file_search_target_list)foreach($lang->file_search_target_list as $__Context->key => $__Context->val){ ?>
...
<?php Context::addJsFile("modules/file/ruleset/deleteChecked.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_modal" id="listManager"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="deleteChecked" />
	<input type="hidden" name="module" value="file" />
	<input type="hidden" name="act" value="procFileAdminDeleteChecked" />
	<input type="hidden" name="page" value="<?php echo $__Context->page ?>" />
	<input type="hidden" name="success_return_url" value="<?php echo getUrl('', 'module', 'admin', 'act', 'dispFileAdminList', 'page', $__Context->page, 'search_target', $__Context->search_target, 'search_keyword', $__Context->search_keyword) ?>" />
	<input type="hidden" name="xe_validator_id" value="modules/file/tpl/file_list/1" />
	<div class="x_modal-header">
		<h1><?php echo $lang->file_manager ?>: <?php echo $lang->delete ?></h1>
	</div>
	<div class="x_modal-body">
		<table width="100%" id="fileManageListTable" class="x_table x_table-striped x_table-hover">
			<caption>
				<strong><?php echo $lang->selected_file ?> <span id="selectedFileCount"></span></strong>
			</caption>
			<thead>
				<tr>
					<th scope="col"><?php echo $lang->file ?></th>
	...
	<div class="x_modal-footer">
		<button type="submit" class="x_btn x_btn-inverse"><?php echo $lang->cmd_delete ?></button>
	</div>
</form>
<script>
jQuery(function($){
	...
	$('a.modalAnchor').bind('before-open.mw', function(){
		if($docTable.find('tbody :checked').length == 0){
			...
			alert('<?php echo $lang->msg_file_cart_is_null ?>');
			return false;
		}
	});
});
