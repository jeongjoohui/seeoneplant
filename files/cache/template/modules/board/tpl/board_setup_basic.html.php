...
<?php Context::addJsFile("modules/board/ruleset/insertBoardForBasic.xml", FALSE, "", 0, "body", TRUE, "") ?><form  id="fo_obj" class="x_form-horizontal" action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertBoardForBasic" />
...
echo $__Context->val->user_id ?>,<?php };
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<textarea name="header_text" id="header_text" class="lang_code" rows="8" cols="42" placeholder="<?php echo $lang->about_header_text ?>"><?php echo htmlspecialchars($__Context->module_info->header_text) ?></textarea>
			<a href="#header_text_help" class="x_icon-question-sign" data-toggle style="vertical-align:top;margin-top:6px"><?php echo $lang->help ?></a>
			<p id="header_text_help" class="x_help-block" hidden><?php echo $lang->about_header_text ?></p>
		</div>
	</div>
	<div class="x_control-group">
		<label class="x_control-label" for="lang_footer_text"><?php echo $lang->footer_text ?></label>
		<div class="x_controls">
			<textarea name="footer_text" id="footer_text" class="lang_code" rows="8" cols="42" placeholder="<?php echo $lang->about_footer_text ?>"><?php echo htmlspecialchars($__Context->module_info->footer_text) ?></textarea>
			<a href="#footer_text_help" class="x_icon-question-sign" data-toggle style="vertical-align:top;margin-top:6px"><?php echo $lang->help ?></a>
			<p id="footer_text_help" class="x_help-block" hidden><?php echo $lang->about_footer_text ?></p>
		</div>
	</div>
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select name="order_target" id="order_target" title="<?php echo $lang->order_target ?>">
				<?php if($__Context->order_target)foreach($__Context->order_target as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->key ?>"<?php if($__Context->module_info->order_target== $__Context->key){ ?> selected="selected"<?php } ?>><?php echo $__Context->val ?></option><?php } ?>
			</select>
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<div class="x_input-append" style="margin-bottom:8px">
				<select name="_admin_member" multiple="multiple" size="<?php echo countobj($__Context->admin_member) ?: 1 ?>">
...
echo $__Context->val->user_id;
			...
			<a href="#adminListHelp" class="x_icon-question-sign"><?php echo $lang->help ?></a>
			<span id="adminListHelp" hidden><?php echo $lang->about_admin_id ?></span>
		</div>
	</div>
	<p style="text-align:right;border-top:1px dotted #ddd;padding:10px 0"><a href="<?php echo $__Context->setupUrl ?>" style="text-decoration:underline"><?php echo $lang->advanced_settings ?></a></p>
</form>
