<?php if(!defined("__XE__"))exit;?><section class="section">
	...
	<?php Context::addJsFile("modules/comment/ruleset/insertCommentModuleConfig.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertCommentModuleConfig" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<label class="x_inline" for="declared_message_admin"><input type="checkbox" name="declared_message[]" id="declared_message_admin" value="admin"<?php if($__Context->comment_config->declared_message && in_array('admin', $__Context->comment_config->declared_message)){ ?> checked="checked"<?php } ?> /> <?php echo $lang->cmd_declared_message_admin ?></label>
				<label class="x_inline" for="declared_message_manager"><input type="checkbox" name="declared_message[]" id="declared_message_manager" value="manager"<?php if($__Context->comment_config->declared_message && in_array('manager', $__Context->comment_config->declared_message)){ ?> checked="checked"<?php } ?> /> <?php echo $lang->cmd_declared_message_manager ?></label>
			</div>
		</div>
